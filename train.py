...
def train(train_data, train_labels):
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            ...
            if i == 0 or (i + 1) % 100 == 0:
                [error_value, y_value] = sess.run([error, y], feed_dict={x: train_data, y_: train_labels})
                endtime = time.clock()
                print("After %d training steps, error is %g, learning rate is %g, time used is %g" % (step, error_value, learning_rate,endtime - starttime))
                # print(y_value)
                data_sum1 = []
                data_sum1.append(i)
                data_sum1.append(error_value)
                data_sum.append(data_sum1)
                np.savetxt("./output/error_summary.txt", data_sum)
                np.savetxt("./output/predict_y.txt", y_value)
...
def main(argv=None):
    inputs = []
    with open('./input/new_input_layer.dat') as f1:
        for line in f1.readlines():
            ...
            inputs.append(temp2)
    labels = []
    with open('./input/new_output_layer.dat') as f2:
